...
export async function GET(req: NextRequest) {
  try {
    const url = new URL(`${BASE_URL}/brands/paginated`);
    ...
    console.error('[GET /api/brands/paginated] Proxy error:', message);
    return NextResponse.json({ message: 'Failed to fetch paginated brands' }, { status: 500 });
  }
}